"""DataUpdateCoordinator for Chenoweth Elementary Menu."""
...
# How many weeks ahead to fetch (this week + next week)
WEEKS_AHEAD = 2


class NutrisliceCoordinator(DataUpdateCoordinator[dict[str, Any]]):
    """Fetches breakfast and lunch menus from Nutrislice.
    ...
    def __init__(self, hass: HomeAssistant) -> None:
        super().__init__(
            ...
            _LOGGER,
            name=DOMAIN,
            # No automatic polling interval – the user drives refreshes via
            # the sync_menu service called from their own HA automation.
    ...
    async def _async_update_data(self) -> dict[str, Any]:
        ...
        for week_start in week_starts:
            try:
                ...
                raise UpdateFailed(f"Network error fetching Nutrislice data: {err}") from err
        ...
        _LOGGER.debug("Nutrislice sync complete – %d days fetched", len(merged))
        return merged
